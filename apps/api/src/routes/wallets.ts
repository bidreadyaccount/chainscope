...
export const walletRoutes: FastifyPluginAsync = async (app) => {
  app.get('/wallets/:address', { schema: { tags: ['wallets'], summary: 'Wallet detail (labels, P&L, bot probability)' } }, async (req) => {
  ...
  app.get('/wallets/:address/trades', { schema: { tags: ['wallets'], summary: 'Wallet trades' } }, async (req) => {
    const { address } = parseOrThrow(walletParamSchema, req.params, 'params');
    const q = parseOrThrow(tradesQuerySchema, req.query, 'query');
    // 404 if wallet unknown; empty feed otherwise.
    if (!(await app.services.wallets.exists(address))) throw notFound(`Wallet ${address} not found`);
  ...
  app.get('/wallets/:address/positions', { schema: { tags: ['wallets'], summary: 'Wallet positions' } }, async (req) => {
    const { address } = parseOrThrow(walletParamSchema, req.params, 'params');
  ...
  app.get('/wallets/:address/relationships', { schema: { tags: ['wallets'], summary: 'Wallet relationships' } }, async (req) => {
    const { address } = parseOrThrow(walletParamSchema, req.params, 'params');
